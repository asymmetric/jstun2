...
public class SdpTools {
			...
			for (Enumeration<MediaDescriptor> e = m_descs.elements(); e.hasMoreElements();) {
				...
				if (prev_md != null) {
					...
					if (spec_attributes.size() == 0
						...
						for (Enumeration<AttributeField> i = spec_attributes.elements(); i
								.hasMoreElements();) {
							AttributeField spec_attr = i
									.nextElement();
							String spec_name = spec_attr.getAttributeName();
							String spec_value = spec_attr.getAttributeValue();
							for (Enumeration<AttributeField> k = prev_attributes.elements(); k
									.hasMoreElements();) {
								AttributeField prev_attr = k
										.nextElement();
	...
	public static SessionDescriptor sdpAttirbuteSelection(
		...
		Vector<MediaDescriptor> new_media = new Vector<MediaDescriptor>();
		for (Enumeration<MediaDescriptor> e = sdp.getMediaDescriptors().elements(); e
				.hasMoreElements();) {
			/* HSC CHANGES END */
